...
@section('page-title', 'Nouveau journal')

@section('content')
    <section class="page pageWide">
        <header class="pageHeader">
            <div class="journalTitleBar">
                <h1 class="pageTitle" id="journal-title">Nouveau journal</h1>
            </div>
        </header>
        ...
        <div class="panel">
            <div class="grid2">
                <div class="field">
                    <label class="fieldLabel">Nom / intitulé du journal</label>
                    <input type="text" id="journal-name" class="input" placeholder="Ex: Journal de caisse" required>
                </div>
            </div>
        </div>
        ...
        <div class="panel">
            <div class="tableHeader">
                ...
                <div class="row gap">
                    <a href="{{ route('journals.index') }}" class="btn">Annuler</a>
                ...
                <table class="table">
                    <thead>
                        ...
                        <tr class="draftRow">
                            ...
                            <th class="mono" id="draft-numero">1</th>
                            <th>
                            ...
                            <th>
                                <input type="text" id="draft-libelle" class="input" placeholder="Libellé de l'écriture"
                                    required>
                            </th>
                            <th>
                                <input type="number" id="draft-debit" class="input right" placeholder="0.00" step="0.01"
                                    min="0">
                            </th>
                            <th>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // Update title when journal name changes
                journalNameInput.addEventListener('input', function() {
                    if (this.value.trim()) {
                        journalTitle.textContent = this.value.trim();
                    } else {
                        journalTitle.textContent = 'Nouveau journal';
                    }
                });
            ...
            `;
                ...
                saveBtn.addEventListener('click', function() {
                    if (!journalNameInput.value.trim()) {
                        errorDiv.textContent = 'Nom du journal obligatoire avant enregistrement.';
                        errorDiv.style.display = 'block';
                        successDiv.style.display = 'none';
                    ...
                    formData.append('journal_name', journalNameInput.value.trim());
                    ...
                    // Submit form
                    fetch('{{ route('journals.saveWithOperations') }}', {
                            method: 'POST',
                            body: formData,
                            headers: {
                        ...
                        .then(data => {
                            if (data.success) {
                                successDiv.textContent = data.message + ' Redirection...';
